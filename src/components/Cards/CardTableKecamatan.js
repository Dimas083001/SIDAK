...
export default function TableKecamatan() {
    ...
    useEffect(() => {
        if (id_kota) {
            const fetchData = async () => {
                try {
                    const response = await axios.get(`https://api.sidak.lampungsehat.org/kecamatan-by-kota/${id_kota}`);
    ...
    useEffect(() => {
        if (prevIdKota) {
            const fetchNamaKota = async () => {
                try {
                    const response = await axios.get(`https://api.sidak.lampungsehat.org/kota/${prevIdKota}`);
    ...
    const handleButtonHapusClick = (nama_kecamatan) => {
        ...
        if (confirmDelete) {
            axios.delete(`https://api.sidak.lampungsehat.org/kecamatan/nama/${nama_kecamatan}`)
                .then(() => {
