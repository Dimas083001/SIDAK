...
export default function TableDK() {
  ...
  const fetchKaderData = async () => {
    try {
      const response = await axios.get('https://api.sidak.lampungsehat.org/kader');
  ...
  const handleSearchChange = async (e) => {
    ...
    try {
      const response = await axios.get(`https://api.sidak.lampungsehat.org/filter-kader?search=${value}`);
  ...
  const handleButtonHapus = async (id) => {
    ...
    if (confirmation) {
      try {
        await axios.delete(`https://api.sidak.lampungsehat.org/kader/${id}`);
