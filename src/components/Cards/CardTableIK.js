...
export default function TableIK({ color }) {
  ...
  useEffect(() => {
    const fetchLaporan = async () => {
      try {
        const response = await axios.get("https://api.sidak.lampungsehat.org/total_laporan_sudah_ik_Perwilayah");
  ...
  useEffect(() => {
    const fetchBelumIKData = async () => {
      try {
        const response = await axios.get("https://api.sidak.lampungsehat.org/menampilkan_status_ik");
