...
export default function FormEditSSR() {
  ...
  try {
    const response = await axios.put(`https://api.sidak.lampungsehat.org/editprofil/${formData.nama_pengguna}`, formData);
    if (response.status === 200) {
      alert("Data updated successfully");
      router.back(); // Redirect ke halaman lain setelah perubahan berhasil
    } else {
      ...
      <div className="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-blueGray-100 border-0">
        <div className="rounded-t bg-white mb-0 px-6 py-6">
          <div className="text-center flex justify-between">
            <h6 className="text-green-700 text-xl font-bold">Edit Profil</h6>
          </div>
        </div>
